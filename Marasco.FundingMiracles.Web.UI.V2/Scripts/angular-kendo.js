...
angular.module('kendo.directives')
    .factory('widgetFactory', ['$parse', '$log', function ($parse, $log) {
      ...
      var mixin = function (kendoWidget, scope, options, attrName, attrValue) {
        ...
        var match = attrName.match(matchExp),
            optionName, fn;

        if (match) {
          // Lowercase the first letter to match the option name kendo expects.
          optionName = match[2].charAt(0)
              .toLowerCase() + match[2].slice(1);
      ...
      var create = function (scope, element, attrs, kendoWidget) {
        ...
        return element[kendoWidget](options)
            .data(kendoWidget);
      };
...
angular.module('kendo.directives')
    .factory('directiveFactory', ['widgetFactory', '$timeout', '$parse',
...
function (widgetFactory, $timeout, $parse) {
  ...
  function exposeWidget(widget, scope, attrs, kendoWidget) {
    if (attrs[kendoWidget]) {
      ...
      var set = $parse(attrs[kendoWidget])
          .assign;
      if (set) {
...
(function (angular) {
  ...
  var widgets = angular.injector(['kendo.directives'])
      .get('kendoWidgets');

  // loop through all the widgets and create a directive
  angular.forEach(widgets, function (widget) {
    angular.module('kendo.directives')
        .directive(widget, ['directiveFactory',
...
angular.module('kendo.directives')
    .directive('kDataSource', [function () {
