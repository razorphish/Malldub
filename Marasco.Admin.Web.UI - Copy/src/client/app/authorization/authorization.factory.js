(function () {
  ...
  function authorizationFactory(
    ...
    var resource = $resource(epAxisSettingsVal.adminApiUrl + '/api/auth/:action', {
      action: '@action'
    }, {
    ...
    var tokenResource = $resource(epAxisSettingsVal.adminApiUrl + '/token', {}, {
      'token': {
        method: 'POST',
    ...
    var _login = function (user, persist) {
      ...
      if (angular.isUndefined(user.userName) || angular.isUndefined(user.password)) {
        ...
        var params = angular.element.param({
          ...
          'grant_type': 'password',
          'client_id':
          angular.isUndefined(user.useRefreshTokens) ? adminConstant.clientId : ''
        });
